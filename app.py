...
# [1] 에러 방지를 위해 변수들을 미리 초기화 (최상단)
if "nums" not in st.session_state: st.session_state["nums"] = [25, 28, 29, 36, 38, 39]
if "bonus" not in st.session_state: st.session_state["bonus"] = 22
if "rid" not in st.session_state: st.session_state["rid"] = 0

# [2] 페이지 설정
st.set_page_config(page_title="Fortune AI", layout="centered")

# [3] CSS (왼쪽 이미지의 골드 디자인 100% 재현)
st.markdown("""
<style>
...
# [5] HTML 템플릿 (f-string 에러를 피하기 위해 단순 더하기 사용)
# 사용자님이 원하시던 '공 숫자'와 '벨소리'를 완벽하게 넣었습니다.
...
if st.button("✨ 다시 분석하기"):
    ...
    st.session_state["rid"] = st.session_state.get("rid", 0) + 1
    st.rerun()
